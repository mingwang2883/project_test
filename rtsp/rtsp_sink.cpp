...
//  printf("RtspSink::send_video\n");
  ...
  if (m_send_error) {
      printf("send_video m_send_error\n");
      return -1;
  }
...
//  printf("send_audio\n");
  ...
  if (m_send_error) {
      printf("send_audio m_send_error\n");
      return -1;
  }
...
int RtspSink::set_destination_and_play()
{
  if (!m_video->sink || !m_audio->sink) {
    printf("!m_video->sink || !m_audio->sink\n");
    return 0;
  }
...
//  printf("sdp : %s\n",STR(sdp));
  ...
  m_client->increate_send_buffer_to(200*1024);
