...
{
  if (get_sockfd() != -1) {
//    printf("Udp::write size : %d\n",size);
    return Udp::write(buf, size, remote);
  }

  int ret = -1;
  FOR_VECTOR_CONST_ITERATOR(TcpStreamRecord *, m_tcp_stream_record, it) {
//    printf("send_rtp size : %d\n",size);
//    printf("(*it)->m_stream_socket_num :%d   (*it)->m_stream_channel_id : %d   size : %d\n",
//           (*it)->m_stream_socket_num,
//           (*it)->m_stream_channel_id,size);
    if (send_rtp_or_rtcp_packet_over_tcp((uint8_t *) buf, size,
          (*it)->m_stream_socket_num, (*it)->m_stream_channel_id) < 0) {
      return -1;
      LOGE("send_rtp_or_rtcp_packet_over_tcp < 0\n");
    } else ret = 0;
  }
  return ret;
}
