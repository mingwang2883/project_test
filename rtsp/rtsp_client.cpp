...
{
//  printf("url_str : %d\n",strlen(STR(surl)));
  char url_str[128] = {0};  //33
  memcpy(url_str,STR(surl),strlen(STR(surl)));
//  printf("url_str : %s\n",url_str);
//  const char *url = STR(surl), *p = strstr(STR(surl), "://");
  const char *url = url_str, *p = strstr(url_str, "://");
  if (!p) {
    LOGE("RTSP url: No :// in url\n");
    return -1;
  }

  if (p-url!=4 || strncasecmp(STR(surl), "rtsp", 4)) {
