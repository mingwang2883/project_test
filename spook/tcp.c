...
static int handle_QT_tunnel( struct req *req, int post )
{
	...
	{
        send_data( req->conn, (unsigned char *)rep, sprintf( rep, "HTTP/1.0 400 Missing session cookie\r\nConnection: close\r\nContent-Type: text/html\r\n\r\n<html><body><p>Broken RTSP/HTTP tunnel request</p></body></html>\n" ) );
	...
	if( ! post )
	{
        ...
        send_data( req->conn, (unsigned char *)rep, sprintf( rep, "HTTP/1.0 200 OK\r\nConnection: close\r\nCache-Control: no-cache\r\nPragma: no-cache\r\nContent-Type: application/x-rtsp-tunnelled\r\n\r\n" ) );
	...
	if( ! cg )
	{
        ...
        send_data( req->conn, (unsigned char *)rep, sprintf( rep, "HTTP/1.0 400 Unknown session cookie\r\nConnection: close\r\nContent-Type: text/html\r\n\r\n<html><body><p>Broken RTSP/HTTP tunnel request</p></body></html>\n" ) );
		req->conn->drop_after = 1;
		return 0;
	}
...
static void do_accept( struct event_info *ei, void *d )
{
    ...
    int snd_size = 300 * 1024;    /* 发送缓冲区大小为8K */
    int optlen = sizeof(snd_size);
    int err = setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &snd_size, optlen);
    if(err<0)
    {
        printf("设置发送缓冲区大小错误\n");
    }
