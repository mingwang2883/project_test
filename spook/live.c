...
static int live_get_sdp( struct session *s, char *dest, int *len,char *path )
{
    struct live_session *ls = (struct live_session *)s->spook_private;
	int i = 0, t;
	...
	for( t = 0; t < MAX_TRACKS && ls->source->track[t].rtp; ++t )
	{
		...
		i += ls->source->track[t].rtp->get_sdp( dest + i, *len - i,
				96 + t, port,
                ls->source->track[t].rtp->spook_private );
...
static struct session *live_open( char *path, void *d )
{
    ...
    ls->sess->spook_private = ls;

	return ls->sess;
}

static void next_live_frame( struct spook_frame *f, void *d )
{
	struct live_track *track = (struct live_track *)d;
	struct live_session *ls, *next;

    if( ! track->rtp->frame( f, track->rtp->spook_private ) )
	{
		unref_frame( f );
		return;
	}
