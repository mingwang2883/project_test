...
void init_frame_heap( int size, int count )
{
	...
	while( count-- > 0 )
	{
        ...
        f->f = (struct spook_frame *)malloc( sizeof( struct spook_frame ) + size );
		f->f->size = size;
...
struct spook_frame *new_frame(void)
{
	...
	if( frame_heap_take->f )
	{
        ...
        f->d = (unsigned char *)f + sizeof( struct spook_frame );
...
static int clone_destructor( struct spook_frame *f, void *d )
{
    unref_frame( (struct spook_frame *)d );
	return 0;
}

struct spook_frame *clone_frame( struct spook_frame *orig )
{
    struct spook_frame *f;
...
void ref_frame( struct spook_frame *f )
{
...
void unref_frame( struct spook_frame *f )
{
	int r;
...
int exchange_frame( struct frame_exchanger *ex, struct spook_frame *frame )
{
	pthread_mutex_lock( &ex->mutex );
...
struct spook_frame *get_next_frame( struct frame_exchanger *ex, int wait )
{
    struct spook_frame *f = NULL;
...
int deliver_frame( struct frame_exchanger *ex, struct spook_frame *f )
{
	unsigned char c = 0;
