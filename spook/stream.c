...
static void convert_uyvy_to_rgb24( struct spook_frame *uyvy, void *d )
{
	struct stream *s = (struct stream *)d;
    struct spook_frame *rgb;
...
static void get_framerate( struct stream *s, int *fincr, int *fbase )
{
	struct stream_destination *dest =
		(struct stream_destination *)s->spook_private;
...
{
	set_waiting( (struct stream_destination *)s->spook_private, running );
}

void deliver_frame_to_stream( struct spook_frame *f, void *d )
{
	struct stream *s = (struct stream *)d;
	struct stream_destination *dest;
					...
					frame_deliver_func convert )
{
	...
	s->spook_private = new_dest( base, convert, s );
	return s;
}
