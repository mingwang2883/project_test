...
extern void ParseNaluData(const unsigned int naluLen,
...
//void MP4Writer::openFile(char *inName, char *tmpdir)
//{
//    GF_ISOFile *file;
...
//        gf_fclose(st);
//    }
//    file = gf_isom_open(inName, (u8) GF_ISOM_OPEN_READ_DUMP , tmpdir);
//}
...
void MP4Writer::FreeAllMem()
{
    ...
    gf_list_del(m_tHEVCNaluParam_SEI.nalus);
    m_tHEVCNaluParam_SEI.nalus = NULL;
...
s32 MP4Writer::Init265(u32 TimeScale)
{
    ...
    memset(&m_tHEVCNaluParam_SEI, 0, sizeof(m_tHEVCNaluParam_SEI));
    ...
    memset(&m_tAVCConfig_SEI,     0, sizeof(m_tAVCConfig_SEI));
    ...
    if (GF_OK != gf_list_add(m_ptHEVCConfig->param_array, &m_tHEVCNaluParam_PPS))
        return -1;
    if (GF_OK != gf_list_add(m_ptHEVCConfig->param_array, &m_tHEVCNaluParam_SEI))
        return -1;
    ...
    if (NULL == m_tHEVCNaluParam_PPS.nalus)
        return -1;
    m_tHEVCNaluParam_SEI.nalus = gf_list_new();
    if (NULL == m_tHEVCNaluParam_SEI.nalus)
        return -1;
    ...
    m_tHEVCNaluParam_SEI.type   =  GF_HEVC_NALU_SEI_PREFIX;
    ...
    m_tHEVCNaluParam_SEI.array_completeness = 1;
    ...
    if (GF_OK != gf_list_add(m_tHEVCNaluParam_PPS.nalus, &m_tAVCConfig_PPS))
        return -1;
    if (GF_OK != gf_list_add(m_tHEVCNaluParam_SEI.nalus, &m_tAVCConfig_SEI))
        return -1;
...
s32 MP4Writer::Write265Sample(u8 *pData, u32 Size, u64 TimeStamp)
{
    ...
    if (CONFIG_STATUS == m_Video265Statue)
    {
        ...
        ptAVCState = (AVCState *)gf_malloc(sizeof(AVCState));
        if (NULL == ptAVCState)
            return -1;
        memset(ptAVCState, 0, sizeof(AVCState));
    }

    while (1)
    {
        pData = FindNalu(pData + NaluSize, Size - (u32)(pData - pStart) - NaluSize,
                         &NaluType, &NaluSize);
        if (NULL == pData)
            break;
        ...
        {
            ParseNaluData(NaluSize-4, pData+4);
            ...
            *((u32 *)pData) = htonl(NaluSize - 4); /* 这里的长度不能包括前四个字节的头！ */
            ...
            //printf(" m_VideoTimeStampStart:%llu, new_time:%llu, diff:%ld \n", m_VideoTimeStampStart, TimeStamp, tISOSample.DTS);
        ...
        else if (CONFIG_STATUS == m_Video265Statue)
        {
            ...
            else if (33 == NaluType && NULL == m_tAVCConfig_SPS.data) /* SPS */
            {
                ...
                gf_isom_set_visual_info(m_ptFile, m_265TrackIndex, m_265StreamIndex,
                                        ptHEVCState->sps[ID].width, ptHEVCState->sps[ID].height);
            }
            else if (34 == NaluType && NULL == m_tAVCConfig_PPS.data) /* PPS */
            {
                ID = gf_media_hevc_read_pps((char *)pData, NaluSize, ptHEVCState);
            ...
            else if (39 == NaluType && NULL == m_tAVCConfig_SEI.data) /* SEI */
            {
                ID = gf_media_avc_reformat_sei((char *)pData, NaluSize, ptAVCState);
                m_tAVCConfig_SEI.id   = ID;
                m_tAVCConfig_SEI.size = (u16)NaluSize;
                m_tAVCConfig_SEI.data = (char *)gf_malloc(NaluSize);
                if (NULL == m_tAVCConfig_SEI.data)
                    continue;
                memcpy(m_tAVCConfig_SEI.data, pData, NaluSize);
            }
            else
            {
                continue;
            ...
            if (m_tAVCConfig_VPS.data && m_tAVCConfig_SPS.data && m_tAVCConfig_PPS.data && m_tAVCConfig_SEI.data)
            {
...
//    printf(" m_AudioTimeStampStart:%llu, new_time:%llu, diff:%ld \n",
//           m_AudioTimeStampStart, TimeStamp, tISOSample.DTS);

    if (GF_OK != gf_isom_add_sample(m_ptFile, m_AACTrackIndex,
                                    m_AACStreamIndex, &tISOSample))
        return -1;
