...
public:
  ...
  long/*off_t*/ cursor() const;
  long/*off_t*/ size() const { return m_filesz; }
  ...
  bool seek_end() const;
  bool seek_ahead(long/*off_t*/ cnt) const;    // cursor move forward
  bool seek_behind(off_t cnt) const;   // cursor move backward
  bool seek_to(long/*off_t*/ pos) const;
