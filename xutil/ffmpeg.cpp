...
CodecParserContext *parser_init(int codec_id)
{
  ...
  LOGE("No parser with codec id: %d\n", codec_id);
  return NULL;
...
int format_find_stream_info(FormatContext *ic)
{
  ...
  for ( ; ; ) {
    ...
    if (read_size >= probesize) {
      ret = count;
      LOGE("Probe buffer size limit of %lld bytes reached\n", probesize);
      break;
    }
  ...
  for (i = 0; i < ic->nb_streams; ++i) {
    st = ic->streams[i];
    if (!has_codec_parameters(st)) {
      LOGW("Could not find codec parameters for stream %d\n", i);
    } else {
      ret = 0;
    }
  }
                              ...
                              Packet *pkt, int flush)
{
  ...
  if (s->max_interleave_delta > 0 &&
    ...
    if (delta_dts > s->max_interleave_delta) {
      LOGE("Delay between the first packet and last packet in the muxing queue is %lld > %lld: forcing output\n",
