...
//               curl_multi_strerror(mc));
  ...
  while (0/*(msg = curl_multi_info_read(multi, &msgs_left))*/) {
...
//  easy = curl_easy_init();
  ...
  if (glob_) {
    ...
    CURLMcode mc = CURLM_OK;//curl_multi_add_handle(glob_->multi, easy);
    if (mc != CURLM_OK) {
...
{
  CURLcode res = CURLE_OK;//curl_easy_perform(easy);
  if (res != CURLE_OK) {
